...
async function page(ctx: Parameters<Command['run']>[0], text: string): Promise<void> {
  ...
  const draw = () => {
    ...
    for (const ln of logical) {
      wrapped.push(...wrapLine(ln, cols));
      if (wrapped.length > top + pageH) break; // wrap only what we might show
    }
    const last = Math.max(0, wrapped.length - pageH);
    top = Math.min(top, last);
    const vis = wrapped.slice(top, top + pageH);
    drawnRows = vis.length;
    s.write(vis.join('\r\n'));
    const atEnd = top >= last && wrapped.length > 0;
    const pct =
      wrapped.length === 0 ? 100 : Math.min(100, Math.round(((top + pageH) / wrapped.length) * 100));
  ...
  await new Promise<void>((resolve) => {
    const off = s.onKey((e) => {
      ...
      // Ctrl-D / Ctrl-U: vim-style paging (same as PageDown / PageUp).
      if (dom.ctrlKey) {
        const c = dom.key.toLowerCase();
        if (c === 'd') top = Math.min(top + state.pageH, state.last);
        else if (c === 'u') top = Math.max(top - state.pageH, 0);
        else return;
        state = draw();
        return;
      }
